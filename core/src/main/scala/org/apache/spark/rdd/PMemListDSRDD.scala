 ...
 */
private[spark] class PMemListDSRDD[T: ClassTag](
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[T] = {
    PMemListDSRDD.loadDSPart(new Path(dsPathStr, PMemRDDLike.PMemPartFileName(split.index)), gfTypes, efProxies, slotKeyId, context)
  }

}

private[spark] object PMemListDSRDD extends Logging {
    ...
    def loadDSPart[T](
      ...
      context: TaskContext): Iterator[T] = {
        ...
        val plist:PersistentLinkedList[BigDataPMemAllocator, T] = 
          PersistentLinkedListFactory.restore(act, efProxies, gfTypes, handler, false)
        plist.setup(act)
        new InterruptibleIterator[T](context, plist.iterator)
    }
}
